...
const Marketproduct = () => {
  ...
  const getProducts = async () => {
    setLoader(true);
    const url = `http://localhost:5800/getProductbyname/${collectionname}`;

    const products = await getProductbynameApi(url);
    // console.log("products: ", products);

    // console.log("products: ", products.product.data.product);

    setSelected(products.product.data.product);
